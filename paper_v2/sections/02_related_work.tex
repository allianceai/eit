...
model's score already places the instance above a tuned operating point; only the
default cutoff is wrong), an axis absent from prior difficulty taxonomies; and (iii) it combines a
...
A parallel critical line reaches a compatible verdict from benchmarking:
Tarawneh et al.\ \cite{tarawneh2022stop} review the oversampling literature and
argue that reported gains are often artefacts of evaluation choices, and
Sa{\u{g}}lam and Cengiz \cite{saglam2026uncertainty} observe that whether
oversampling appears to help depends strongly on \emph{which} metric is
consulted. Our decomposition explains where this metric dependence comes from:
an operating-point move registers on threshold-sensitive metrics (balanced
accuracy, recall, G-mean) while leaving ranking metrics (AUC) unchanged, so the
verdict on oversampling is inherited from the metric's threshold sensitivity.
...
extensions. Geometric SMOTE \cite{douzas2019geometric} replaces linear
interpolation altogether: it draws synthetic points inside a (possibly
truncated and deformed) hypersphere around each minority seed whose radius can
be bounded by the distance to the \emph{nearest majority} instance, an explicit
geometric guard against generating in the other class's territory, and hence a
design-level response to precisely the boundary-crossing mass that
Proposition~\ref{prop:cross}(iii) identifies as the harmful quantity; we test
it directly, on the controlled boundary and at scale
(\S\ref{sec:results:mechanism}, \S\ref{sec:results:frontier}). The
85-variant survey of Kov\'acs \cite{kovacs2019smote}
...
(Proposition~\ref{prop:cross}). It is complementary to, not in conflict with, their
clean-margin analysis.

\paragraph{Uncertainty-guided oversampling.}
A distinct line replaces geometric seeding heuristics with \emph{uncertainty}
signals that decide where to generate. Ertekin \cite{ertekin2013adaptive}
adaptively oversamples the informative region near the decision boundary;
CGMOS \cite{zhang2016cgmos} weights candidate seeds by the certainty change a
synthetic point would induce on both classes; Aguiar and Cano
\cite{aguiar2023active} allocate an active-learning budget to oversampling in
partially labeled imbalanced streams; and Tao \cite{tao2026local} targets
high-local-entropy regions adversarially. Closest to our decomposition,
Sa{\u{g}}lam and Cengiz \cite{saglam2026uncertainty} separate the two
uncertainty sources explicitly: their generator flags high-\emph{aleatoric}
instances as noise and excludes them from seeding, then directs generation
toward high-\emph{epistemic} instances. That design choice is operationally
convergent with our findings, that generation seeded in the (aleatoric)
overlap region is what harms accuracy, that excluding such seeds removes the
harm (clean-masked SMOTE), and that only the epistemic (data-reducible) slice
of the deficit benefits from more data. The difference is the object of study:
these works propose \emph{new oversamplers} and evaluate them as methods,
whereas we supply the instance-level diagnosis and the mechanism that explain
\emph{why} aleatoric-region generation hurts
(Proposition~\ref{prop:cross}, \S\ref{sec:results:interventional}), and our
remedy decomposition shows the epistemic slice such methods can address is
small ($\sim$6\% of the minority deficit) next to the threshold-recoverable
majority ($\sim$68\%), which needs no generation at all.
