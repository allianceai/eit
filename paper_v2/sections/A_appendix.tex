...
class ratio is the identical $k$-NN computation. Two design notes: (i)~members
are unweighted, because class-balanced members enact the operating-point shift
...
$\ge 5$ minority errors). The threshold-recoverable share, the paper's
headline, is stable across instruments ($68$--$69\%$; per-dataset correlation
with the RF instrument $r=0.68$ / $0.55$). The non-tree instruments assign
more of the residue to noise (TCP-only gate, note (ii)).}
\label{tab:nontree}
\input{tables/table_nontree.tex}
\end{table}

The threshold-recoverable share is essentially unchanged (Table~\ref{tab:nontree}):
$69\%$ for bagged MLPs and $68\%$ for bagged logistic regression against $69\%$
for the RF instrument, rising to $77\%$ on $\mathrm{IR}>3$ datasets for both
(RF: $74\%$). The decomposition's ordering, threshold-recoverable far above
everything else, is instrument-independent.
...
fractions move by at most $2.3$\,pp between $M{=}5$ and $M{=}20$, and the
threshold-recoverable share by $2.3$\,pp on average ($3.7$\,pp for $M{=}10$);
...
Below $M{=}5$ the decomposition is visibly unstable. $M{=}5$ therefore sits at
