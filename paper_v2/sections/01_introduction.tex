...
A natural response is to ask \emph{which} resampling method a given dataset
needs. Recent work shows that data characteristics --- not the imbalance ratio
alone --- moderate oversampling effectiveness \cite{jiang2026beyond}, and a line
of meta-learning research recommends a resampler from dataset meta-features
\cite{carvalho2025resampling}. These efforts are
\emph{correlational}: they relate measured complexity to observed performance,
but do not explain \emph{why} a method helps, and their selection rules barely
beat the trivial ``always SMOTE'' baseline (e.g.\ \cite{jiang2026beyond} reports
its rule \emph{below} always-SMOTE). What is missing is a \emph{mechanistic} account
of the tradeoff that tells us which lever to pull.

We provide one. We argue the tradeoff is \emph{not} inherent to oversampling but
...
\emph{out-of-bag} random-forest predictions via an aleatoric--epistemic
uncertainty decomposition \cite{shaker2020aleatoric,hullermeier2021aleatoric}:
...
minority --- sorting each error into threshold-recoverable, data-reducible
(epistemic), or irreducible (aleatoric) --- and find the deficit is overwhelmingly
\textbf{threshold-recoverable} (mean $69\%$ across 79 datasets, $74\%$ at
$\mathrm{IR}>3$): the classifier already ranks these instances correctly. The known
...
and worse calibration, and once \emph{every} strategy is given the same
...
\item \textbf{A remedy decomposition of the minority deficit (headline).} We
  decompose the \emph{misclassified} minority into the remedy that would fix each
  error --- \emph{threshold-recoverable}, \emph{data-reducible} (epistemic), or
  \emph{irreducible} (aleatoric) --- by combining an aleatoric--epistemic
  uncertainty decomposition with a threshold-recoverability test. Unlike prior
  ...
  ($69\%$ on average, $74\%$ at $\mathrm{IR}>3$).
...
\item \textbf{Evidence at scale (confirmation, not claim).} Across 79 datasets and
  ...
  discrimination gain (AUC unchanged) and is worse-calibrated, and that once
  ...
  $\pm1$\,pp across three base learners --- the apparent dominance of non-generative
